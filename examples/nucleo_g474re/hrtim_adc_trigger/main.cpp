...
// PCLK2. Prescaler::Div4 selects a 42.5 MHz Timer A time base.
//
//     42.5 MHz / 42'500 = 1 kHz
//
// CHA1 on PA8 is configured as a 50% duty PWM reference. ADC trigger 1 fires
...
constexpr Hrtim1::Value period = 42'500;
...
configureAdc()
{
	...
	const bool channelsConfigured =
		Adc1::setPinChannel<RegularInput>(sampleTime) &&
		Adc1::setInjectedConversionSequenceLength(2) &&
		Adc1::setInjectedConversionChannel<InjectedInput>(0, sampleTime) &&
		Adc1::setInjectedConversionChannel<RegularInput>(1, sampleTime);
...
main()
{
	...
	MODM_LOG_INFO << "modm example: STM32G4 HRTIM-triggered ADC" << modm::endl;
	...
	while (true)
	{
		...
		if (count >= nextReport)
		{
			...
			MODM_LOG_INFO << "regular=" << count << " adc1_in1=" << regular
						  << " injected=" << injected << " adc1_in2=" << injectedA1
						  << " adc1_in1=" << injectedA0 << " errors=" << errors << modm::endl;
		}
