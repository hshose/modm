...
int main()
{
	...
	while (true)
	{
		...
		if (frameCount == 0) {
			MODM_LOG_WARNING << "FIFO contained no complete accel+gyro frame. bytes=" << bytesRead << modm::endl;
			continue;
		}

		auto readAxis = [&](std::size_t index) {
			return static_cast<int16_t>(
				uint16_t(fifoBuffer[index]) |
				(uint16_t(fifoBuffer[index + 1]) << 8));
		};

		MODM_LOG_INFO << "FIFO bytes read: " << bytesRead
					  << " (frames=" << frameCount << ")" 
					  << modm::endl;
		...
		for (std::size_t frameIndex = 0; frameIndex < frameCount; ++frameIndex)
		{
			...
			accData.raw = modm::Vector3i(
				readAxis(frameOffset + FifoAccOffset + 0),
			...
			MODM_LOG_INFO << "  [" << frameIndex << "] x: " << acc[0]
						  << " y: " << acc[1]
						  << " z: " << acc[2] << modm::endl;
		}
		...
		for (std::size_t frameIndex = 0; frameIndex < frameCount; ++frameIndex)
		{
			...
			gyroData.raw = modm::Vector3i(
				readAxis(frameOffset + FifoGyroOffset + 0),
			...
			MODM_LOG_INFO << "  [" << frameIndex << "] x: " << gyro[0]
						  << " y: " << gyro[1]
						  << " z: " << gyro[2] << modm::endl;
		}
