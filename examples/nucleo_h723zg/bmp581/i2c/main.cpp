...
initializeBaro()
{
	...
	const auto intConfig = Baro::IntConfig::Enable |
	                       Baro::IntConfig::Polarity;  // Active high
...
main()
{
	...
	while (true)
	{
		...
		if (baro.readData(data)) {
			...
			const float altitude = 44330.0f * (1.0f - powf(pressureHpa / seaLevelPressure, 0.1903f));
			...
			if (readErrorCount % 50 == 0) {
				MODM_LOG_ERROR << "Failed to read sensor data!\n";
			}
			LedRed::set();
		}
