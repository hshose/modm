...
public:
	...
	static constexpr uint8_t httpText[] = {
			...
			"\r\n"
			"<html><body><h1>Hello from your STM32H723!</h1></body></html>"
	};
	...
	run(void *parameter)
	{
		...
		if (buffer) {
			...
			while (true) {
				...
				const int32_t bytes = FreeRTOS_recv(connectedSocket, buffer, ipconfigTCP_MSS, 0);
				if (bytes <= 0)
					break;
		...
		const TickType_t shutdownTime { xTaskGetTickCount() };
		do {
...
public:
	...
	run(void *)
	{
		Socket_t listeningSocket = FreeRTOS_socket(FREERTOS_AF_INET,
...
vApplicationIPNetworkEventHook(eIPCallbackEvent_t eNetworkEvent)
{
	...
	if (not taskCreated) {
		xTaskCreate(HttpServerListener::run, HttpServerListener::name,
				configMINIMAL_STACK_SIZE * 2, 0, configMAX_PRIORITIES - 2, 0);
		taskCreated = true;
	}
