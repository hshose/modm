...
struct ethernet
{
	...
	modm_aligned(32)
	modm_section(".bss_d2_sram1")
	static uint8_t RxBuffers[RX_BUFFER_NUMBER][RX_BUFFER_SIZE];

	modm_aligned(32)
	modm_section(".bss_d2_sram2")
	static uint8_t TxBuffers[TX_BUFFER_NUMBER][TX_BUFFER_SIZE];
	...
	{
#if (__DCACHE_PRESENT == 1U)
		if (not isDCacheEnabled() or address == nullptr or size == 0)
			return;

		const uintptr_t start = uintptr_t(address) & ~(CacheLineSize - 1);
		const uintptr_t end = (uintptr_t(address) + size + CacheLineSize - 1) & ~(CacheLineSize - 1);
	...
	DMARxDescListInit()
	{
		...
		for (BaseType_t index = 0; index < RX_BUFFER_NUMBER; ++index) {
			...
			descriptor.DESC3 = RxDescriptorReady_t(
					RxDescriptorReady::DmaOwned |
					RxDescriptorReady::InterruptOnCompletion |
					RxDescriptorReady::Buffer1Valid).value;
	...
	clearTxBuffers()
	{
		...
		for (std::size_t index = 0; index < count; ++index) {
			...
			if ((DmaTxDescriptorToClear->DESC3 & uint32_t(TxDescriptor3::DmaOwned)) != 0)
				break;
	...
	emacInterfaceInput()
	{
		...
		while ((dmaRxDescriptor->DESC3 & uint32_t(RxDescriptorStatus::DmaOwned)) == 0) {
			...
			__DMB();
			dmaRxDescriptor->DESC3 = RxDescriptorReady_t(
					RxDescriptorReady::DmaOwned |
					RxDescriptorReady::InterruptOnCompletion |
					RxDescriptorReady::Buffer1Valid).value;
	...
	phyCheckLinkStatus(bool hasReceived)
	{
		...
		if (xTaskCheckForTimeOut(&phyLinkStatusTimer, &phyLinkStatusRemaining)) {
			...
			phyLinkStatusRemaining = pdMS_TO_TICKS(
					phyLinkStatus == eth::LinkStatus::Up ?
						PhyLinkStatusHighMs : PhyLinkStatusLowMs);
		}
...
modm_aligned(32)
modm_section(".bss_d2_sram1")
uint8_t ethernet::RxBuffers[RX_BUFFER_NUMBER][RX_BUFFER_SIZE];
modm_aligned(32)
modm_section(".bss_d2_sram2")
uint8_t ethernet::TxBuffers[TX_BUFFER_NUMBER][TX_BUFFER_SIZE];
...
xNetworkInterfaceInitialise()
{
	...
	if (ethernet::initStatus == ethernet::InitStatus::Init) {
		...
		if (not xTaskCreate(ethernet::emacHandlerTask, "EMAC", ethernet::emacTaskStackDepth, nullptr,
	...
	if (EMAC::getLinkStatus() == modm::platform::eth::LinkStatus::Up) {
		EMAC::enableInterrupt(EMAC::Interrupt_t(
				...
				EMAC::Interrupt::AbnormalIrqSummary |
...
xNetworkInterfaceOutput(NetworkBufferDescriptor_t * const descriptor, BaseType_t releaseAfterSend)
{
	...
	do {
		...
		ethernet::cleanDCache(
				ethernet::TxBuffers[dmaTxDescriptor - ethernet::DmaTxDescriptorTable],
				transmitSize);

		dmaTxDescriptor->DESC0 = uint32_t(
				ethernet::TxBuffers[dmaTxDescriptor - ethernet::DmaTxDescriptorTable]);
		...
		dmaTxDescriptor->DESC3 =
				ethernet::TxDescriptor3_t(
						ethernet::TxDescriptor3::FirstSegment |
						ethernet::TxDescriptor3::LastSegment).value |
				(transmitSize & ethernet::FrameLengthMask);
#if (ipconfigDRIVER_INCLUDED_TX_IP_CHECKSUM == 1)
		dmaTxDescriptor->DESC3 |= ethernet::TxDescriptor3_t(
				ethernet::TxDescriptor3::ChecksumFull).value;
#endif
